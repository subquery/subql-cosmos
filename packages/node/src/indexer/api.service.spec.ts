...
import { CosmosProjectNetworkV0_3_0 } from '@subql/common-cosmos';
import { GraphQLSchema } from 'graphql';
import { omit } from 'lodash';
import { SubqueryProject } from '../configure/SubqueryProject';
import { ApiService } from './api.service';

jest.mock('@cosmos/api', () => {
  const ApiPromise = jest.fn();
  (ApiPromise as any).create = jest.fn(() => ({
    on: jest.fn(),
    runtimeChain: jest.fn(),
    runtimeVersion: { specName: jest.fn() },
    // genesisHash for cosmos/Juno
    genesisHash: '',
    consts: jest.fn(),
  }));
  // dunno if wsProvider is needed
  return { ApiPromise, WsProvider: jest.fn() };
});

// what is the correct testNetwork type to use?
const testNetwork: CosmosProjectNetworkV0_3_0 = {
